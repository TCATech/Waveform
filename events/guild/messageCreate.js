...
module.exports = async (client, message) => {
  ...
  if (command) {
    if (
      client.settings.get(message.guild.id, "music.channels") ===
      message.channel.id
    )
      return message
        .reply(
          `${client.allEmojis.x} **Please use that command somewhere else!**`
        )
        .then((msg) => {
          setTimeout(() => {
            try {
        ...
        return message.reply({
          embeds: [
            new Discord.MessageEmbed()
              ...
              .setFooter(ee.footertext, ee.footericon)
              .setTitle(
                `${client.allEmojis.x} **You are not allowed to use this command here!**`
              )
              .setDescription(
                `Please use it in one of these channels:\n> ${botchannels
                  .map((c) => `<#${c}>`)
                  .join(", ")}`
              ),
          ],
