  ...
  run: async (client, message, args) => {
    if (args[0]) {
      ...
      if (!command) {
        ...
        if (!category || category === "favorites" || category === "filters") {
        ...
        if (category === "music") {
          embed
            .setTitle(
              `🎶 Music commands [${
                client.commands.filter(
                  (cmd) =>
                    cmd.directory === "music" ||
                    cmd.directory === "favorites" ||
                    cmd.directory === "filters"
                ).size
              }]`
            )
            .setDescription(
            ...
            .addField("\u200B", "__**Related categories:**__")
            .addField(
              "💖 Favorites",
              client.commands
                .filter((cmd) => cmd.directory === "favorites")
                .map((cmd) => `\`${cmd.name}\``)
                .sort((a, b) => a.localeCompare(b))
                .join(" ")
            )
            .addField(
              "❔ Filters",
              client.commands
                .filter((cmd) => cmd.directory === "filters")
                .map((cmd) => `\`${cmd.name}\``)
                .sort((a, b) => a.localeCompare(b))
                .join(" ")
            );
          ...
          new MessageSelectMenu()
            ...
            .addOptions(
              categories
                .filter(
                  (cmd) =>
                    cmd.directory !== "Favorites" && cmd.directory !== "Filters"
                )
                .map((cmd) => {
                  return {
                    label: cmd.directory,
                    ...
                    description: `Commands from the ${cmd.directory.toLowerCase()} category.`,
                  };
                })
            )
        ),
      ...
      collector.on("collect", (int) => {
        ...
        if (directory.toLowerCase() === "music") {
          categoryEmbed
            .setTitle(
              `🎶 Music commands [${
                client.commands.filter(
