...
module.exports = {
  ...
  aliases: ["fwd"],
  description: "Skip forwads in the song for X seconds.", //the command description for Slash Command Overview
  cooldown: 10,
          ...
          return message.reply({
            embeds: [
              new MessageEmbed()
                .setColor(ee.wrongcolor)
                .setFooter(ee.footertext, ee.footericon)
                .setTitle(`${client.allEmojis.x} **Please add a duration!**`)
                .setDescription(
                  `**Usage:**\n> \`${client.settings.get(
                    guild.id,
                    "prefix"
                  )}forward <Duration_in_Sec>\``
                ),
            ],
          });
        }
        ...
        message.reply({
          embeds: [
            new MessageEmbed()
              ...
              .setTimestamp()
              .setTitle(
                `⏩ **Skipped forward in the song by \`${seekNumber} seconds\`!**`
              )
              .setFooter(
